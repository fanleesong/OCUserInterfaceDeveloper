...
@synthesize commentsCount,  unread, hasReply;
...
- (SinaStatus*)initWithJsonDictionary:(NSDictionary*)dic {
	if (self = [super init]) {
		...
		if (userDic) {
			user = [[SinaUser userWithJsonDictionary:userDic] retain];
		}
...
{
	return [[[SinaStatus alloc] initWithJsonDictionary:dic] autorelease];
}
